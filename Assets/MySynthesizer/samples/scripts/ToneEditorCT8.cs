...
public class ToneEditorCT8 : MonoBehaviour
{
    ...
    {
        Text text = obj.transform.Find(name + "/Text").GetComponent<Text>();
        Slider slider = obj.transform.Find(name + "/Slider").GetComponent<Slider>();
        disabledEvent = true;
        text.text = label + val;
        slider.minValue = min;
        slider.maxValue = max;
    ...
    private void loadTone(ToneParam tone)
    {
        param = tone.Clone();
        {
            GameObject obj = transform.Find("Panel/LFO").gameObject;
        ...
        {
            GameObject obj = transform.Find("Panel/ENV").gameObject;
        ...
        {
            WaveTable wt = transform.Find("Panel/WaveTable").GetComponent<WaveTable>();
            for (int i = 0; i < 32; i++)
            {
                wt.WT[i] = param.WT[i];
            }
    ...
    void Awake()
    {
        ...
        {
            Keyboard keyboard = transform.Find("Panel/Keyboard").GetComponent<Keyboard>();
            keyboard.ChNo = ChNo;
            keyboard.PortNo = PortNo;
        }
        {
            Transform mmlEditorCanvas = transform.parent.Find("MMLPlayerCanvas");
            Button button = transform.Find("Panel/MMLEditorButton").GetComponent<Button>();
