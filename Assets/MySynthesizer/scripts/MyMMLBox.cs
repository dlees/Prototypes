...
namespace MySpace
{
    ...
    public class MyMMLBox : MonoBehaviour, IEnumerable
    {
        ...
        public IEnumerator<Tuple<int, MyMMLClip>> GetEnumerator()
        {
            for(int i = 0; i< clips.Count; i++)
            {
                yield return new Tuple<int, MyMMLClip>(i, clips[i]);
            }
        }
        ...
        public bool TryGetValue(string name, out List<Tuple<int, MyMMLClip>> clips)
        {
            clips = new List<Tuple<int, MyMMLClip>>();
            if (name == null)
            {
                return false;
            }
            for (int i = 0; i < this.clips.Count; i++)
            {
                ...
                if (clip.Name.Equals(name))
                {
                    clips.Add(new Tuple<int, MyMMLClip>(i, clip));
                }
            }
